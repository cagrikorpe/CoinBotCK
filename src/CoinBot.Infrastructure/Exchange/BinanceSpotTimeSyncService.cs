...
public sealed class BinanceSpotTimeSyncService(
    HttpClient httpClient,
    IMemoryCache memoryCache,
    TimeProvider timeProvider,
    IOptions<BinancePrivateDataOptions> options,
    ILogger<BinanceSpotTimeSyncService> logger) : IBinanceSpotTimeSyncService
{
    private const string CacheKey = "binance-spot-server-time-sync";
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            cachedState is not null)
        {
            return BuildSnapshot(
                ...
                localAppTimeUtc,
                "CachedFallback",
                "Binance spot server time probe yenilenemedi; son başarılı offset kullanılmaya devam ediyor.");
        }
        ...
        return new BinanceTimeSyncSnapshot(
            ...
            FailureReason: "Binance spot server time probe başarısız olduğu için offset üretilemedi.");
    }
    ...
    private async Task<CachedTimeSyncState?> RefreshAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            using var response = await httpClient.GetAsync("api/v3/time", timeoutCts.Token);
            ...
            {
                throw new InvalidOperationException("Binance spot server time response did not contain a valid serverTime.");
            }
        ...
        {
            logger.LogDebug(exception, "Binance spot server time probe failed.");
            return null;
        }
    }
