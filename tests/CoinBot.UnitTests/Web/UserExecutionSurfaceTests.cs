...
public sealed class UserExecutionSurfaceTests
{
    ...
    public void PositionsSurface_RendersUltraSoftUserExecutionFlow()
    {
        ...
        Assert.Contains("data-cb-manual-close-form", content, StringComparison.Ordinal);
        Assert.Contains("data-cb-manual-close-action=\"ManualClose\"", content, StringComparison.Ordinal);
        Assert.Contains("data-cb-user-manual-close-button", content, StringComparison.Ordinal);
        Assert.Contains("data-cb-manual-close-submit", content, StringComparison.Ordinal);
        Assert.Contains("data-cb-user-manual-close-checkbox", content, StringComparison.Ordinal);
        Assert.Contains("data-cb-manual-close-double-confirm", content, StringComparison.Ordinal);
        Assert.Contains("type=\"checkbox\"", content, StringComparison.Ordinal);
        Assert.Contains("@Html.AntiForgeryToken()", content, StringComparison.Ordinal);
        Assert.Contains("Reduce-only close emrini onayliyorum ve pozisyonu kapat komutunu bilerek confirm ediyorum.", content, StringComparison.Ordinal);
        ...
        Assert.Contains("name=\"accountScope\"", content, StringComparison.Ordinal);
        Assert.DoesNotContain("multiple", content, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("data-cb-admin-multi-symbol-runtime", content, StringComparison.Ordinal);
        Assert.DoesNotContain("data-cb-admin-multi-symbol-runtime-table", content, StringComparison.Ordinal);
        Assert.DoesNotContain("OwnerUserId", content, StringComparison.Ordinal);
        Assert.DoesNotContain("ExchangeAccountId", content, StringComparison.Ordinal);
        Assert.DoesNotContain("CorrelationId", content, StringComparison.Ordinal);
        Assert.DoesNotContain("api-secret", content, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("x-mbx-apikey", content, StringComparison.OrdinalIgnoreCase);
