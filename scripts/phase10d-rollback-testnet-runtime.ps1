...
$exchangeAccount = $snapshot.ExchangeAccount
...
if ($null -ne $exchangeAccount -and -not [string]::IsNullOrWhiteSpace([string]$exchangeAccount.Id)) {
    $sql += @"

UPDATE ExchangeAccounts
SET CredentialStatus = $(ConvertTo-SqlStringLiteral $exchangeAccount.CredentialStatus),
    UpdatedDate = $(ConvertTo-SqlDateTimeLiteral $exchangeAccount.UpdatedDate)
