...
"@
    ...
    $tempFile = Join-Path $env:TEMP ("phase10g-verify-" + [Guid]::NewGuid().ToString("N") + ".sql")
...
if ($blocker -eq "PrivatePlaneStale") {
    Write-Fail -Blocker "PrivatePlaneStaleNotAccepted" -LatestOrderId $latestOrderId -LatestEnvironment $latestEnvironment -LatestExecutorKind $latestExecutorKind
}
