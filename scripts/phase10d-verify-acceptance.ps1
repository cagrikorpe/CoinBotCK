...
if ($marker.PSObject.Properties.Name -contains "RequireLiveWorker") {
    $requireLiveWorker = [bool]$marker.RequireLiveWorker
}
...
if ($requireLiveWorker -and ($null -eq $heartbeat -or $null -eq $heartbeat.LastHeartbeatAtUtc)) {
